...
class TelethonUserBot:
	...
	async def start(self):
		...
		try:
			...
			logger.info(f"✅ تم تسجيل الدخول كـ: {me.first_name} (@{getattr(me, 'username', '')})")
			
			self._running = True
			logger.info("✅ تم بدء يوزر البوت بنجاح!")
			
			# بدء مراقبة الرسائل في الخلفية
	...
	async def _handle_yt_command(self, event: Message):
		...
		try:
			async with self._semaphore:
				...
				user_db_id = await self._ensure_user_exists(event.sender_id)
...
تواصل مع الدعم الفني
	...
	async def _handle_settings_command(self, event: Message):
		...
		try:
			...
			async with db_manager.get_session() as session:
				user = (await session.execute(select(User).where(User.telegram_id == user_tg_id))).scalars().first()
				if not user:
					await event.reply("❌ لم يتم العثور على بيانات المستخدم.")
					return
...
# ملاحظة: لا تنشئ مثيلاً عاماً هنا لتجنّب طلب API_ID/API_HASH أثناء الاستيراد